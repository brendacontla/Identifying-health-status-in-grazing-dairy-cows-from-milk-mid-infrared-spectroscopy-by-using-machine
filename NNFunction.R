...
nnFunction.d1<-function(y,nt,data,n,n.col,k,weights){
  sensi.model<-c()
  speci.model<-c()
  roc.model<-c()
  accu.model<-c()
  ...
  if(y=="sick"){
    ...
    model <- keras_model_sequential() %>% 
      layer_dense(units = 64, input_shape = 858) %>%
      layer_activation_leaky_relu() %>%
      layer_dense(units = 2, activation = "sigmoid")
    ...
    model <- keras_model_sequential() %>% 
      layer_dense(units = 64, input_shape = 858, activation = "relu") %>%
      layer_dense(units = 2, activation = "sigmoid") 
  ...
  for (i in 1:k) {
    ...
    #Neural Network Model
    modelo<-model %>% fit(spectra.train, y.train, batch_size = 40, epochs = 60, validation_split=0.2, class_weight=weights)
...
nnFunction.d4<-function(y,nt,data,n,n.col,k,weights){
  sensi.model<-c()
  speci.model<-c()
  roc.model<-c()
  accu.model<-c()
  ...
  if(y=="lameness"){
    model <- keras_model_sequential() %>% 
      layer_dense(units = 64, input_shape = 852) %>%
      layer_activation_leaky_relu() %>%
      layer_dense(units = 2, activation = "sigmoid")  
    
  }else{
    model <- keras_model_sequential() %>% 
      layer_dense(units = 32, input_shape = 852) %>%
      layer_activation_leaky_relu() %>%
      layer_dense(units = 2, activation = "sigmoid") 
